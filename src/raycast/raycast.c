...
/*   Updated: 2022/11/12 10:43:24 by mmarinel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_fl	ft_initial_delta(t_fl p_ax_pos, t_fl p_dir_ax_pos);
int	ft_ray_step_size(t_fl p_dir_ax_pos);
...
static t_raycast_data ft_ray_data_init(t_game *game, float ray_angle)
{
	...
	t_raycast_data	rc_data = (t_raycast_data) {
		ray_dir,
		t_fl_div(t_fl_new(1), t_fl_abs(ray_dir.x)),
		t_fl_div(t_fl_new(1), t_fl_abs(ray_dir.y)),
...
{
	{
		raycast_info->hit_point.x
			= t_fl_sum(
				game->player_pos.x,
				t_fl_mult(game->player_dir.x, raycast_info->euclidean_dist)
				);
	}
	{
		raycast_info->hit_point.y
			= t_fl_sum(
				game->player_pos.y,
				t_fl_mult(game->player_dir.y, raycast_info->euclidean_dist)
				);
	}
}
...
{
	const t_fl	dist_nhp_through_x = t_fl_diff(rc_data.dist_nhp_through_x, rc_data.delta_x);
	const t_fl	dist_nhp_through_y = t_fl_diff(rc_data.dist_nhp_through_y, rc_data.delta_y);
	if (rc_data.side == e_VERTICAL)
		rc_ret->euclidean_dist = dist_nhp_through_x;
	else
		rc_ret->euclidean_dist = dist_nhp_through_y;
	rc_ret->perp_dist = t_fl_mult(
		rc_ret->euclidean_dist,
		t_fl_new( sin(ray_angle))
		);
}
...
t_fl	ft_initial_delta(t_fl p_ax_pos, t_fl p_dir_ax_pos)
{
	...
	{//*						can be inf !!!
		p_ax_pos = t_fl_div(
			t_fl_new(ft_int_abs(sq_ax_pos + 1 - p_ax_pos.val)),
			t_fl_abs(p_dir_ax_pos)
		);
	}
	else
	{//*					CANNOT be inf !!!
		p_ax_pos = t_fl_div(
			t_fl_new(ft_int_abs(p_ax_pos.val - sq_ax_pos)),
			t_fl_abs(p_dir_ax_pos)
		);
	}
	return (p_ax_pos);
...
void	ft_print_ray_data(t_raycast_data rc_data)
{
	...
	printf("delta_x: "); ft_print_t_fl(rc_data.delta_x); printf("\n");
	printf("delta_y: "); ft_print_t_fl(rc_data.delta_y); printf("\n");
	...
	printf("dist_nhp_through_x: "); ft_print_t_fl(rc_data.dist_nhp_through_x); printf("\n");
	printf("dist_nhp_through_y: "); ft_print_t_fl(rc_data.dist_nhp_through_y); printf("\n");
