...
/*   Updated: 2022/12/06 12:45:08 by mmarinel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "line_drawing.h"

static void	bresenham_algo(t_int_2d_point vertices[2],
			t_data *canvas_data, t_next_pixel_f next_pixel_f, t_nxt_px_f_arg *nxt_px_f_arg);
 ...
 * 
 * @param vertices the ordered set of vertices that defines the segment to be drawn
 * @param canvas_data the image on which to draw
 * @param next_pixel function that prints the next pixel on the segment
 * @param arg the argument to pass on each call to the next_pixel function
 */
static void	bresenham_algo(t_int_2d_point vertices[2],
			t_data *canvas_data, t_next_pixel_f next_pixel_f, t_nxt_px_f_arg *nxt_px_f_arg)
{
	const size_t			norm_delta_x = abs(vertices[1].x - vertices[0].x);
	const size_t			norm_delta_y = abs(vertices[1].y - vertices[0].y);
	int						d_k;//*		the decision variable
	t_int_2d_point			cur_point;
	t_bres_line_type		bres_type;

	nxt_px_f_arg->cur_px = &cur_point;
	bres_type = bres_get_type(vertices[0], vertices[1], norm_delta_x, norm_delta_y);
	...
	while (e_false == bres_eol(cur_point, vertices[1], bres_type))
	{
