...
/*   Updated: 2022/12/24 18:48:59 by earendil         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	render_health_bar(t_game *g)
{
	...
	const size_t		horizontal_offset = (
		(g->screen_handle.width * 10.0f) / 100
		);
...
void	render_mana_bar(t_game *g)
{
	...
	const size_t		horizontal_offset = (
		(g->screen_handle.width * 25.0f) / 100
		);
	...
	)
{
	...
	{
		px.x = 0;
		while (px.x < (int)width)
		{
			ft_put_px_to_image(
				&g->screen_handle.frame_data,
				ft_get_pixel_offset(
					&g->screen_handle.frame_data,
					(t_int_2d_point){px.x + offset->x, px.y + offset->y}
					),
				*bar_color
			);
			px.x += 1;
